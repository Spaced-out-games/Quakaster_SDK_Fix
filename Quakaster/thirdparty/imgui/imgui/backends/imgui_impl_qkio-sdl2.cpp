...
#include "imgui_impl_qkio-sdl2.h"
...
/*
bool ImGui_ImplSDL2_ProcessEvent(const qk::io::Event& evt)
{
    ImGui_ImplSDL2_Data* bd = ImGui_ImplSDL2_GetBackendData();
    ...
    // --- Mouse movement ---
    if (evt.header.category == EEventCategory::MOUSE_MOVE_EVENT)
    {
        if (evt.header.type.mouse_move_event == EMouseMoveEvent::MOUSE_MOVE)
        {
            if (ImGui_ImplSDL2_GetViewportForWindowID(evt.payload.mouse_move_event.window_ID) == nullptr)
                return false;

            io.AddMouseSourceEvent(ImGuiMouseSource::ImGuiMouseSource_Mouse);
            io.AddMousePosEvent(
                evt.payload.mouse_move_event.delta_pos.x,
                evt.payload.mouse_move_event.delta_pos.y
            );
            return true;
        }
        return false;
    ...
    // --- Mouse wheel ---
    if (evt.header.category == EEventCategory::MOUSE_WHEEL)
    {
    ...
    // --- Mouse buttons ---
    if (evt.header.category == EEventCategory::MOUSE_BUTTON_EVENT)
    {
        if (evt.header.type.mouse_event == EMouseEvent::MOUSE_BUTTON_PRESS ||
            evt.header.type.mouse_event == EMouseEvent::MOUSE_BUTTON_RELEASE)
        {
            win_id = (uint32_t)evt.payload.mouse_event.windowID;
            if (ImGui_ImplSDL2_GetViewportForWindowID(win_id) == nullptr)
                return false;

            switch (evt.payload.mouse_event.button)
            {
            ...
            down = (evt.header.type.mouse_event == EMouseEvent::MOUSE_BUTTON_PRESS);
            io.AddMouseSourceEvent(ImGuiMouseSource::ImGuiMouseSource_Mouse);
            io.AddMouseButtonEvent(mouse_button, down);

            bd->MouseButtonsDown = down ? (bd->MouseButtonsDown | (1 << mouse_button))
    ...
    // --- Text input ---
    if (evt.header.category == EEventCategory::TEXT_INPUT)
    {
        // TODO: Handle SDL_TEXTINPUT
        return false;
    }

    // --- Key press/release ---
    if (evt.header.category == EEventCategory::KEY_PRESS_EVENT)
    {
        if (evt.header.type.key_event == EKeyEvent::KEY_PRESS ||
            evt.header.type.key_event == EKeyEvent::KEY_RELEASE)
        {
            win_id = (uint32_t)evt.payload.key_event.window_ID;
            if (ImGui_ImplSDL2_GetViewportForWindowID(win_id) == nullptr)
                return false;

            ImGui_ImplSDL2_UpdateKeyModifiers((SDL_Keymod)evt.payload.key_event.modifiers);
            key = ImGui_ImplSDL2_KeyEventToImGuiKey(
                evt.payload.key_event.keycode,
                (SDL_Scancode)evt.header.variant
            );

            io.AddKeyEvent(key, (evt.header.type.key_event == EKeyEvent::KEY_PRESS));
    ...
    // --- Window events ---
    if (evt.header.category == EEventCategory::WINDOW_EVENT)
    {
